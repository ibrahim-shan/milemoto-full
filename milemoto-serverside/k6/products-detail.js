...
export function setup() {
  ...
  const listRes = http.get(`${baseUrl}/api/v1/admin/products?page=1&limit=50`, {
...
export default function (data) {
  ...
  const res = http.get(`${baseUrl}/api/v1/admin/products/${id}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  ...
  if (useValid) {
    ...
    check(res, { "product detail 4xx": (r) => r.status === 404 || r.status === 400 });
  }
