...
// Get collection
router.get('/:id', requirePermission('collections.read'), async (req, res, next) => {
  try {
...
// Create collection
router.post('/', requirePermission('collections.manage'), async (req, res, next) => {
  try {
...
// Update collection
router.put('/:id', requirePermission('collections.manage'), async (req, res, next) => {
  try {
    const id = z.coerce.number().int().min(1).parse(req.params.id);
...
// Delete collection
router.delete('/:id', requirePermission('collections.manage'), async (req, res, next) => {
  try {
    const id = z.coerce.number().int().min(1).parse(req.params.id);
...
router.post('/:id/products', requirePermission('collections.manage'), async (req, res, next) => {
  try {
    const id = z.coerce.number().int().min(1).parse(req.params.id);
...
router.post('/:id/preview', requirePermission('collections.read'), async (req, res, next) => {
  try {
    const id = z.coerce.number().int().min(1).parse(req.params.id);
    const body = z
      .object({
...
router.get('/:id/products', requirePermission('collections.read'), async (req, res, next) => {
  try {
    const id = z.coerce.number().int().min(1).parse(req.params.id);
