...
export async function addItem(
    ...
    const [existing] = await db
        ...
        .from(cartitems)
        .where(
            and(eq(cartitems.cartId, cartId), eq(cartitems.productVariantId, data.productVariantId))
        )
        .limit(1);
...
export async function mergeGuestCart(
    ...
    for (const guestItem of data.items) {
        ...
        const qty = Math.min(guestItem.quantity, MAX_QTY);
...
export async function validateCart(userId: number): Promise<CartResponse> {
    ...
    const itemsToRemove = cart.items.filter(
        (item) => item.warning?.includes('no longer available') || item.warning?.includes('out of stock')
    );
